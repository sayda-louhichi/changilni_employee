...
Api networkHandler = Api();
        ...
        body: Form(
          ...
          children: [
            Container(
              ...
              cinTextField(hint:"Cin",icon:Icons.vpn_key),
                ...
                onTap: () async {
                  ...
                  if (_formkey.currentState.validate() && validate) {
                    ...
                    var responseRegister =
                        await networkHandler.post("/employee/register", data);
  ...
  checkUser() async {
    if (_emailController.text.length == 0) {
      ...
      var response = await networkHandler
          .get("/employee/checkuser/${_emailController.text}");
      if (response['Status']) {
        setState(() {
          // circular = false;
          validate = false;
  ...
  Widget cinTextField({controller,hint,icon}) {
    return Container(
      ...
      padding: EdgeInsets.only(left: 10),
        child:TextFormField(
            ...
            validator: (value) {
              if (value.isEmpty) return "Cin can't be empty";
